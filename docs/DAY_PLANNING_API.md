...
Formularze webu i mobile nie pytają o widoczność: wydarzenie z `teamId` zapisują zawsze jako `TEAM`, a bez zespołu jako `PRIVATE`. Edycja starszego wydarzenia zespołu z `PRIVATE`, także pojedynczego wystąpienia, wysyła `visibility: "TEAM"`. API nadal przyjmuje `PRIVATE` z `teamId`; korzysta z tego import planu lekcji, który wpisuje lekcje do kalendarza ucznia.
