...
export default function DayPlanning({ user }) {
    ...
    const fromDate = view === 'mine' && period === 'week' ? weekStart(date) : date;
    const toDate = addDays(fromDate, view === 'mine' && period === 'week' ? 7 : 1);
    ...
    if (editing) return <div className="day-planning"><DayEventEditor key={`${editing.event?.id || 'new'}-${editing.occurrenceKey || 'series'}-${editing.event?.version || 0}`} {...editing} zone={zone} user={user} teams={teams} onClose={() => setEditing(null)} onReload={async () => {
    ...
    return <section className="day-planning">
        ...
        <div className="day-toolbar">
            <div className="day-date-controls"><IconButton icon="chevronLeft" label={t('dayPlanning.previous')} onClick={() => setDate(addDays(date, view === 'mine' && period === 'week' ? -7 : -1))} /><label className="day-date-input"><span className="sr-only">{t('dayPlanning.date')}</span><input aria-label={t('dayPlanning.date')} type="date" required value={date} onChange={(e) => { if (e.target.value) setDate(e.target.value); }} /></label><IconButton icon="chevronRight" label={t('dayPlanning.next')} onClick={() => setDate(addDays(date, view === 'mine' && period === 'week' ? 7 : 1))} /><Button type="button" variant="text" onClick={() => setDate(dateInZone(new Date(), zone))}>{t('dayPlanning.today')}</Button></div>
            <div className="day-toolbar-selects">{view === 'mine' && <label className="day-compact-field">{t('dayPlanning.period')}<select aria-label={t('dayPlanning.period')} value={period} onChange={(e) => setPeriod(e.target.value)}><option value="day">{t('dayPlanning.day')}</option><option value="week">{t('dayPlanning.week')}</option></select></label>}{<label className="day-compact-field">{t(view === 'mine' ? 'dayPlanning.tagCatalog' : 'dayPlanning.team')}<select aria-label={t(view === 'mine' ? 'dayPlanning.tagCatalog' : 'dayPlanning.team')} value={teamId} onChange={(e) => setTeamId(e.target.value)}><option value="">{t(view === 'mine' ? 'dayPlanning.personalCatalog' : 'dayPlanning.personal')}</option>{teams.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}<label className="day-compact-field">{t('dayPlanning.displayZone')}<select aria-label={t('dayPlanning.displayZone')} value={zone} onChange={(e) => setZone(e.target.value)}>{[...new Set([browserTimeZone(), 'Europe/Warsaw', 'Europe/London', 'America/New_York', 'UTC'])].map((value) => <option key={value}>{value}</option>)}</select></label></div>
        </div>
        ...
        <div id="day-content" role="tabpanel" aria-labelledby={`day-tab-${view}`}>
            {view !== 'planner' ? <>
                <h3 className="day-range-title">{displayDate(fromDate, i18n.language, { weekday: 'long' })}{period === 'week' && view === 'mine' ? ` – ${displayDate(addDays(toDate, -1), i18n.language)}` : ''}</h3>
                {loading || actionBusy && !detail ? <p role="status" className="day-loading">{t('common.loading')}</p> : error ? <div role="alert" className="day-alert">{error}<Button type="button" variant="text" onClick={reload}>{t('dayPlanning.retry')}</Button></div> : view === 'team' && !teamId ? <p className="day-empty">{t('dayPlanning.chooseTeam')}</p> : <DayCalendar events={calendar.events || []} busy={calendar.busy || []} date={fromDate} mode={view === 'mine' ? period : 'team'} people={shownPeople} zone={zone} onOpen={showDetail} />}
