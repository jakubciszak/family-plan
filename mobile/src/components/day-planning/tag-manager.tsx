...
export default function TagManager({ tags, team, onChanged, onClose }: {
  ...
  const save = async () => {
    ...
    try {
      await saveCalendarTag({ name: name.trim(), color, scope: scope as CalendarTag['scope'], teamId: scope === 'TEAM' ? editing?.teamId ?? team?.id ?? null : null }, editing?.id);
  ...
  return <View style={{ gap: 12 }} testID="day-tag-manager">
    ...
    {tags.map((tag) => <View key={tag.id} style={{ gap: 4 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}><TagColorDot color={tag.color} /><Text variant="titleMedium" style={{ flex: 1 }}>{tag.name} · {t(`dayPlanning.tagScopes.${tag.scope}`)}</Text></View>
    ...
    <ChoicePicker label={t('dayPlanning.tagScope')} value={scope} disabled={saving || !!editing}
      options={[{ value: 'PERSONAL', label: t('dayPlanning.tagScopes.PERSONAL') }, ...(team?.role === 'admin' || editing?.scope === 'TEAM' ? [{ value: 'TEAM', label: t('dayPlanning.tagScopes.TEAM') }] : [])]} onChange={setScope} />
